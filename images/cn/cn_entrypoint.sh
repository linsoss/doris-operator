...
CN_PROBE_INTERVAL=${CN_PROBE_INTERVAL:-2}
# timeout for probe cn: 60 seconds
CN_PROBE_TIMEOUT=${CN_PROBE_TIMEOUT:-60}
...
override_be_conf() {
  ...
  # make sure node role is computation
...
add_self() {
  ...
  while true; do
    doris_note "Try to add myself($SELF_HOST:$HEARTBEAT_PORT) to cluster as BACKEND(Compute Node)..."
